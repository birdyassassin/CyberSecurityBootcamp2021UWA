...
1. Add user accounts for `sam`, `joe`, `amy`, `sara`, and `admin`.
    ...
    - Command to add each user account (include all five users): sudo useradd -m [username]
...
2. Add users `sam`, `joe`, `amy`, and `sara` to the managed group.
    ...
    - Command to add users to `engineers` group (include all four users): sudo usermod -a -G engineers [username]
